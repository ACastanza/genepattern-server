...
public class GenomeSpaceBean {
    ...
    public GenomeSpaceBean() {
        ...
        genomeSpaceEnabled = new Boolean(System.getProperty("genomeSpaceEnabled"));
  ...
  public String submitRegistration() {
    ...
    public void deleteFileFromGenomeSpace(ActionEvent ae){
    	...
    	System.out.println("Delete from GS " + dirnameParam + "/" + filenameParam);
    	GsFile theFile = getFile(dirnameParam, filenameParam);
    	HttpSession httpSession = UIBeanHelper.getSession();
     ...
     * @return
     */
    public GsFile getFile(String dirname, String file){
    	for (GSDirectory dir: availableDirectories){
    		if ((dir.getName().equals(dirname)) || (dirname == null)){
    			for (GSFileInfo aFile: dir.getGsFiles()){
    				if (aFile.getFilename().equals(file)) return aFile.getGsFile();
    			}
    ...
    public String getFileURL(String dirname, String filename){
    	if (filename == null) return null;
     	GsFile theFile = getFile(dirname, filename);
    	HttpSession httpSession = UIBeanHelper.getSession();
     	try {
     		GsSession sess = (GsSession)httpSession.getAttribute(GS_SESSION_KEY);
	...
	private void saveFileToGenomeSpace(HttpSession httpSession, File in) {
		GsFile gsFile = new GsFile(in);
    	try {
    		GsSession sess = (GsSession)httpSession.getAttribute(GS_SESSION_KEY);
    		sess.uploadFile(gsFile);
    		this.setMessageToUser( "File uploaded to GS " + gsFile.getFilename() );
    		this.availableDirectories = null;
    		
    	} catch (InternalServerException e){
    		e.printStackTrace();
    		this.setMessageToUser( "There was a problem uploadeing the file to GS, " + gsFile.getFilename() );
    	}
	}
	
	
	
    public List<GSDirectory> availableDirectories;
    
    /**
     * get the list of directories in GenomeSpace this user can look at
     * @return
     */
    public List<GSDirectory> getAvailableDirectories(){
    	
    	if ((availableDirectories == null) || (availableDirectories.size() == 0)) {
    		availableDirectories = new ArrayList<GSDirectory>();
    	HttpSession httpSession = UIBeanHelper.getSession();
      	GsSession gsSession = (GsSession)httpSession.getAttribute(GS_SESSION_KEY);
		...
		/**
		 * The directory layer is unnecessary for now but will be needed once we have
		 * subfolders and shared folders in GS
		 */
		GSDirectory userDir = new GSDirectory(gsUser.getUsername());
		availableDirectories.add(userDir);
		
		if (gsSession.isLoggedIn()){
			List<GsFile> gsFiles;
			try {
				gsFiles = gsSession.list();
				userDir.setGsFileList(gsFiles, this.kindToModules, this);
    ...
    public void setSelectedModule(String selectedModule) {
    	...
    	List<GSDirectory> dirs =  getAvailableDirectories();
    	...
    	for (GSDirectory aDir : dirs) {
    	    List<GSFileInfo> outputFiles = aDir.getGsFiles();
    	    if (outputFiles != null) {
    		for (GSFileInfo o : outputFiles) {
