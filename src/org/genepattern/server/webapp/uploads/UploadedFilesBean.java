...
public class UploadedFilesBean {
    ...
    public void deleteFile(ActionEvent ae){
    	...
    	System.out.println("Delete from uploads " + dirnameParam + "/" + filenameParam);
    ...
    public String getFileURL(String dirname, String filename){
    	...
    	return "http://127.0.0.1:8080/gp/getFile.jsp?task=&file="+dirname+"/" + filename;
    ...
    /**
     * to help us detect when the file list should be updated.  Not very efficient at this point
     * @return
     */
    ...
    public List<UploadDirectory> getAvailableDirectories(){
		...
		for (File f: tmp.listFiles(nameFilt)){
			for (String aFile: f.list()){
				...
				ufi.setGenePatternUrl(getGenePatternFileURL(f.getName(), aFile));
